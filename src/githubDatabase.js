...
const GitProfs = () => {
  ...
  var [DatatoShow, updateDatatoShow] = new useState({
    login: "sunitaverma12 ",
    id: "90575699",
    html_url: "https://github.com/sunitaverma12",
    avatar_url: "https://avatars.githubusercontent.com/u/47671000?v=4",
  });
  const fetcher = async () => {
  ...
  return (
    <div>
      <h1 style={{ textAlign: "center" }}>This is for Api Call </h1>
      <input
        type="text"
        value={user}
      ...
      />
      <button onClick={fetcher} style={{ marginRight: "15px" }}>
        {" "}
        Click to fetch the data
      </button>
      <SuCard
        Login={DatatoShow.login}
        Id={DatatoShow.id}
        Url={DatatoShow.html_url}
        Img={DatatoShow.avatar_url}
      />
    </div>
  );
};
